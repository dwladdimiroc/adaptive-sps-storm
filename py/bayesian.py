...
def bayesian_prediction(samples, prediction_number):
    t = list(range(len(samples)))

    bayesian = linear_model.BayesianRidge().fit(np.array(t).reshape(-1, 1), samples)

    predictions = []
    for x in range(prediction_number):
        predictions.append(bayesian.predict(np.array([len(samples) + x]).reshape(-1, 1))[0])
